...
def save_images(response, folder_name=images_default_folder_name):
    """Saves all images from the api response in the specified folder, naming the images by the card id."""
    ...
    for card in json_response["data"]:
        ...
        for image_key in image_key_folder.keys():
            for images in card["card_images"]:
                ...
                if files.file_exists(image_key_folder[image_key], card_id, image_formats):
